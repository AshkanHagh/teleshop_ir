...
export const StarContextProvider = ({
  children
}: {
  children: React.ReactNode
}) => {
  const [starIndex, setStarIndex] = useState<number>(0)
  const [fetchStars, { data, error, isLoading, setData }] =
    useGetServiceOptions<Star[]>("star")

  // Animated numbers for smooth UI updates (springs)
  ...
  // Socket error → toast notification
  const handleSocketError = useCallback(
    () => toast.error("خطا در بروزرسانی قیمت!"),
    []
  )

  // Handle incoming WebSocket messages with star price updates
  const handleSocketMessage = useCallback(
    (message: MessageEvent<string>) => {
      const socketData: SocketData = JSON.parse(message.data)
      if (socketData.type !== "updated-star-prices") return

      // Build lookup for new prices by star id
      ...
      // Update context data with new prices from socket
      setData((prev) => {
        ...
        const updatedService: Star[] = prev.service.map((star) => {
          ...
          return {
            ...star,
            // Random value addition simulates fluctuation
            irrPrice:
              priceData.totalTonPriceInIrr + Math.floor(Math.random() * 10),
  ...
  useEffect(() => {
    if (!data) return
    // Animate numbers whenever star index changes or data updates
