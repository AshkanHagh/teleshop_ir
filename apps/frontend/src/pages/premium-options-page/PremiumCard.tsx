...
const TGPremiumCard: React.FC<TGPremiumCardProps> = ({
  id,
  icon,
  duration,
  features,
    ...
    <>
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 flex flex-col h-full">
        <div className="flex items-center mb-4">
          {getIcon(icon)}
          <h2 className="text-xl font-semibold text-gray-800 mb-1 mr-1">
            {duration}
          </h2>
        </div>
        ...
        <p className="text-lg font-bold text-gray-800 mb-2">{ton} TON</p>
        <p className="text-sm text-gray-600 mb-4">
          {irr.toLocaleString()} ريال
        </p>

        <Button onClick={() => setShowModal(true)}>انتخاب</Button>
      </div>
