...
import type {
  PaymentRequest,
  PaymentVerification,
} from "zarinpal-checkout-v4/lib/types";
...
export const createOrderService = async (
  ...
  try {
    ...
    if (!servicePrice)
      throw ErrorFactory.ResourceNotFoundError("Service not found");
    console.log(servicePrice);

    const { irr, ton } = servicePrice;

    const zarinpalCheckout = await zarinpal.requestPayment(<PaymentRequest>{
      amount: irr,
...
export const verifyPaymentService = async (
  authority: string,
  paymentStatus: "OK" | "NOK",
) => {
  try {
    if (paymentStatus !== "OK") {
      throw ErrorFactory.PaymentFailedError(
        authority,
        "The payment process has been ended",
      );
    }

    const orderDetail = (await RedisQuery.jsonGet(
      RedisKeys.pendingOrder(authority),
      "$",
    )) as string | null;
    if (!orderDetail) {
      throw ErrorFactory.PaymentFailedError(
        authority,
        "The payment process failed. no pending order founded",
      );
    }

    const { irr, serviceId, serviceName, ton, userId, username } = JSON.parse(
      orderDetail,
    )[0] as PendingOrderPayload;

    const isPaymentDetailValid = await zarinpal.verifyPayment(<
      PaymentVerification
    >{ amount: irr, authority });
