...
export const signInAndSignup = CatchAsyncError(async (context: Context) => {
  ...
  const userDetail: SelectUser =
    await validateTelegramAuthService(userSafeData);
...
export const refreshToken = CatchAsyncError(async (context: Context) => {
  ...
  if (!refreshToken)
    throw ErrorFactory.TokenRefreshError("No refresh token cookie exists");
