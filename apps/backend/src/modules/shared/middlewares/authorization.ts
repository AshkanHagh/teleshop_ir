...
export const isAuthenticated = async (
  ...
  try {
    ...
    const { id: userId } = (await verifyJwtToken(
      accessToken,
      env.ACCESS_TOKEN,
    )) as TokenPayload;
    if (!userId)
      throw ErrorFactory.AccessTokenInvalidError(
        `Invalid access token: ${accessToken}`,
      );

    const isUserCashed = (await RedisQuery.jsonGet(
  ...
  return CatchAsyncError(async (context: Context, next: Next) => {
    ...
    if (
      !authorizedRoles.some((authorizedRole) =>
        roles.some((role) => authorizedRole === role),
      )
    ) {
