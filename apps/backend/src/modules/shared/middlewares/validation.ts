...
type ReqSourceMapParsing<Source> = Record<
  ...
  (
    reqPayload: Source,
    schema: ZodSchema<Source>,
  ) => z.SafeParseReturnType<unknown, unknown>
>;

export const validationPayload = <Source>(
  source: RequestEntries,
  schema: ZodSchema,
) => {
  return async (context: Context, next: Next) => {
    try {
      ...
      const safeParse = (): z.SafeParseReturnType<unknown, unknown> =>
        schema.safeParse(reqPayload);
      ...
      if (!validatedReqPayload.success) {
        throw ErrorFactory.ValidationError(
          validatedReqPayload.error?.issues[0].message,
        );
      }
