...
export const availableServices_service = async (): Promise<
  SelectServices[]
> => {
  try {
    ...
    let availableServices = isServicesCashed
      ? isServicesCashed[0]
      : await getServices();

    if (!availableServices)
      throw ErrorFactory.ResourceNotFoundError("No services found");
